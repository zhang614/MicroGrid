...
def graphMicroGrid(batteryState, buyEnergy=[], sellEnergy=[], sampleTime=60, nowDayTime=None,
                   supply=[], demand=[], costList=[], zoneList=[], title=""):  # , wasted, useGrid):
    global fig, ax1
    fontSize = 14
    ...
    axarr[2].set_ylabel('KW hour')
    axarr[2].set_title("\n" + title + ' Buy Sell Schedule', fontsize=fontSize)
    barBuy = axarr[2].bar(timeIterations, buyEnergy, 1.0,
                          alpha=0.9,
                          color='salmon',
                          linewidth=0,
                          antialiased=True)
    barSell = axarr[2].bar(timeIterations, sellEnergy, 1.0,
                           alpha=0.9,
                           color='lime',
                           linewidth=0,
                           antialiased=True)
    axarr[2].legend([barBuy[0], barSell[0]], ['Buy', 'Sell'], loc=1)
    axarr[2].grid(axis='both', color='black', linestyle='-', linewidth=1, alpha=0.5)
    # Cumulate value
    axarr[3].set_xlabel('Time in Hours')
    axarr[3].set_ylabel(' $ Value')
    axarr[3].set_title(title + ' Operational Cost/Benefit', fontsize=fontSize)
    axarr[3].plot(costList, ls='steps', c='b')
    toColor = {'off': 'yellowgreen', 'mid': 'gold', 'peak': 'brown', '': 'white'}
    for i in range(0, len(zoneList)):
        axarr[3].axvspan(i - 1.5, i - 0.5, facecolor=toColor[zoneList[i]], alpha=0.2)
    legends = []
    ...
    axarr[3].set_xticks(xPositions)
    axarr[3].set_xticklabels(xLabels)
    axarr[3].legend(handles=legends, loc=1)
    axarr[2].grid(axis='both', color='black', linestyle='-', linewidth=1, alpha=0.5)
